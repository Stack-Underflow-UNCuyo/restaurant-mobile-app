...
import type { Carta, SeccionCarta } from "@/types/entities";
import { cartaService } from "@/services/cartaService";
import { seccionCartaService } from "@/services/seccionCartaService";
import toast from "react-hot-toast";

type FormData = { seccionCartaId: string; fechaDesde: string; fechaHasta: string };
const emptyForm: FormData = { seccionCartaId: "", fechaDesde: "", fechaHasta: "" };
const noErrors = { seccionCartaId: false, fechaDesde: false, fechaHasta: false };

export default function CartaTable() {
  const [items, setItems] = useState<Carta[]>([]);
  const [seccionesCartaOptions, setSeccionesCartaOptions] = useState<{ value: string; label: string }[]>([]);
  ...
  useEffect(() => {
    Promise.all([cartaService.getAll(), seccionCartaService.getAll()])
      .then(([cartas, secciones]) => {
        setItems(cartas);
        setSeccionesCartaOptions(secciones.map((s: SeccionCarta) => ({ value: String(s.id), label: s.nombre })));
      })
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId !== null) {
        const updated = await cartaService.update(editingId, formData);
  ...
  return (
    <>
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {["Id", "Sección Carta", "Fecha Desde", "Fecha Hasta", "Acciones"].map((h) => (
              ...
              {loading ? (
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="px-5 py-4 text-gray-800 text-theme-sm dark:text-white/90">
                    {seccionesCartaOptions.find((o) => o.value === String(item.seccionCartaId))?.label ?? item.seccionCartaId ?? "-"}
                  </TableCell>
      ...
      <DeletionConfirmationPopUp isOpen={isConfirmOpen} onClose={closeConfirm} onConfirm={confirmDelete} isLoading={deleting}
        description="¿Estás seguro de que deseas eliminar esta carta? Esta acción no se puede deshacer." />
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-md p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Select options={seccionesCartaOptions} placeholder="Seleccionar sección" defaultValue={formData.seccionCartaId}
              onChange={(value) => { setFormData((prev) => ({ ...prev, seccionCartaId: value })); if (errors.seccionCartaId) setErrors((prev) => ({ ...prev, seccionCartaId: false })); }} />
            {errors.seccionCartaId && <p className="mt-1.5 text-xs text-error-500">Debe seleccionar una sección de carta</p>}
          </div>
          <div>
