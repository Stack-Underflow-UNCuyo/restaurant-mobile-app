      ...
      <div className="flex flex-col gap-4 px-4 py-6 sm:px-6 md:px-8">
        {carta.seccionesCarta.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-3">
            <SeccionCard seccion={carta.seccionesCarta[0]} cartaId={cartaId} featured index={0} />
            <MenuOptionCard href={`/carta/${cartaId}/menu`} index={1} />
            {carta.seccionesCarta.slice(1).map((seccion, i) => (
              <SeccionCard key={seccion.id} seccion={seccion} cartaId={cartaId} index={i + 2} />
            ))}
          </div>
        )}
      </div>
