...
public class ContactoTelefonicoService extends BaseService<ContactoTelefonico> {
    ...
    // Regex para validar teléfonos (permite números, espacios, guiones y un '+' inicial opcional)
    ...
    @Override
    protected void validar(ContactoTelefonico contacto, String caso) throws Exception {
        try {
            if (contacto.getTipoContacto() == null) {
                throw new ErrorServiceException("Debe indicar qué tipo de contacto es (Personal, Laboral o Empresarial)");
            }
            ...
            switch (caso) {
                ...
                case "UPDATE": {
                    Optional<ContactoTelefonico> existenteOptional =
                            contactoRepository.findByTelefonoAndPersona_IdAndTipoContactoAndTipoTelefonoAndEliminadoFalse(
                    ...
                    if (existenteOptional.isPresent()) {
                        ...
                        if (!contactoExistente.getId().equals(contacto.getId())) {
                            throw new ErrorServiceException("Ya existe otro contacto con el mismo teléfono, tipo de contacto y tipo de teléfono para esta persona");
                        }
                    }
                    break;
                }
